...
pub fn write_spr_file(
    ...
    for sprite in sprites {
        if sprite.is_empty {
            ...
            // Write RGB header (3 bytes, all zeros for now)
            file.write_all(&[0u8, 0u8, 0u8])
...
pub fn update_sprites_in_spr(
    ...
    // If adding new sprites beyond current count, expand the address table
    // WARNING: This logic assumes we can overwrite what follows the address table.
    // If the file is tightly packed, this WILL corrupt the first sprite data.
    // TODO: Implement proper file shifting or require full rewrite for expansion.
    if sprites_count > existing_sprite_count {
        let new_entries_count = sprites_count - existing_sprite_count;

        // Append zero addresses for the new sprites
        file.seek(SeekFrom::Start(header_size + (existing_sprite_count * 4) as u64))
            .map_err(|e| format!("Failed to seek to address table end: {}", e))?;

        let zero_addresses = vec![0u8; (new_entries_count * 4) as usize];
        file.write_all(&zero_addresses)
            .map_err(|e| format!("Failed to expand address table: {}", e))?;
    }
